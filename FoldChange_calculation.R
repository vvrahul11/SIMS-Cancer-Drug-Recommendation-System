calculate_fold_change <- function(node_names, data_foldChange){
...
node_names = c("Her_pathway", "CDK4_6", "PLK_AURKA_Kinesins", "ANGIOGENESIS", "ANGIOPOIETINS", "IMMUNO-Modulator", "PI3K", "MET",
               ...
               "JAK_STAT", "HEDGEHOG", "FGF", "DNA_REPAIR","OTHERS")
...
final_data_gene_expression = calculate_fold_change (node_names, data_foldChange)
write.csv(final_data_gene_expression, file = "~/Dropbox/winther/Data/final_data_gene_expression.csv")  

data_foldChange =read.csv("mir_rev_path-31Jan2014-2.csv", as.is = T)
final_data_miRNA = calculate_fold_change (node_names, data_foldChange)
